...
import (
	"github.com/volodimyr/leetcode/ds"
)

type LFUCache struct {
	maxCapacity int
	minFreq     int
	nodes       map[int]*ds.Dnode
	freqList    map[int]*ds.DoublyLinkedList
}

func Constructor(cap int) LFUCache {
	return LFUCache{
		maxCapacity: cap,
		nodes:       make(map[int]*ds.Dnode),
		freqList:    make(map[int]*ds.DoublyLinkedList),
	}
}
...
func (c *LFUCache) Put(k int, v int) {
	...
	node := ds.NewDnode(k, v, 1)
	c.nodes[k] = node
	if c.freqList[1] == nil {
		c.freqList[1] = ds.NewDoublyLinkedList()
	}
	c.minFreq = 1
	c.freqList[1].InsertHeadByRef(node)
}

func (c *LFUCache) increaseFreq(node *ds.Dnode) {
	freq := node.Freq()
	...
	if c.freqList[node.Freq()] == nil {
		c.freqList[node.Freq()] = ds.NewDoublyLinkedList()
	}
